<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">Editar disciplina</h2>
    </x-slot>

    <div class="py-8">
        <div class="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('disciplinas.update', $disciplina) }}" class="space-y-5 rounded-lg bg-white p-6 shadow-sm dark:bg-gray-800">
                @csrf
                @method('put')
                ...
                <div>
                    ...
                    <x-text-input id="professor" name="professor" type="text" class="mt-1 block w-full" :value="old('professor', $disciplina->professor)" required />
                    <x-input-error class="mt-2" :messages="$errors->get('professor')" />
                </div>

                <div>
                    <x-input-label for="horario" value="Horário" />
                    <x-text-input id="horario" name="horario" type="datetime-local" class="mt-1 block w-full" :value="old('horario', $disciplina->horario->format('Y-m-d\\TH:i'))" required />
